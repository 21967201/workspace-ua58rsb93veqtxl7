"""
LLMLingua-2 简化实现 (修复版)

修复内容:
1. 正则表达式字符串 (添加rl前缀)
2. 导入语句 (移除numpy依赖)
3. 函数调用 (修复self.batch_compress)
"""
...
class SimpleLLMLingua:
    ...
    def _select_important_tokens(
        ...
        for i, token_idx in enumerate(scores.keys()):
            if token_idx not in keep_indices:
                # 获取token (从keys中)
...
def compare_with_phase6():
    ...
    for i, text in enumerate(test_texts, 1):
        ...
        llmlingua_ratio = 1 - (len(llmlingua_compressed) / len(text)) if len(text) > 0 else 0
        ...
        phase6_ratio = 1 - (len(phase6_compressed) / len(text)) if len(text) > 0 else 0
        ...
        print(f"  [对比]")
        if llmlingua_time > 0:
            speed_diff = phase6_time / llmlingua_time
        else:
            speed_diff = float('inf')
        
        print(f"    压缩率: LLMLingua {llmlingua_ratio:.1%} vs Phase 6 {phase6_ratio:.1%}")
        print(f"    速度: Phase 6 比 LLMLingua 快 {speed_diff:.2f}x")
        print()
