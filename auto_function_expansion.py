...
### 功能列表
""" + "\n".join([f"""{i}. **{result['feature']}** ({result['complexity']复杂度)
   - **描述**: {result['description']}
   - **开发时间**: {result['development_time']:.2f}小时
   - **成功率**: {result['success_rate']:.2f}
   - **状态**: {result['status']}
""" + (f"   - **预测准确性**: {result['prediction_accuracy']:.2f}\n" if result.get('prediction_accuracy') else "") for i, result in enumerate(feature_results, 1)]) + f"""
### 统计结果
...
""" + "\n".join([f"""{i}. **{result['feature']}** ({result['complexity']复杂度)
   ...
   - **成功率**: {result['success_rate']:.2f}
   - **状态**: {result['status']}
""" + (f"   - **调整效果**: {result['adjustment_effectiveness']:.2f}\n" if result.get('adjustment_effectiveness') else "") for i, result in enumerate(adjustment_results, 1)]) + f"""
### 统计结果
...
""" + "\n".join([f"""{i}. **{result['feature']}** ({result['complexity']复杂度)
   ...
   - **成功率**: {result['success_rate']:.2f}
   - **状态**: {result['status']}
""" + (f"   - **分析准确性**: {result['analysis_accuracy']:.2f}\n" if result.get('analysis_accuracy') else "") for i, result in enumerate(behavior_results, 1)]) + f"""
### 统计结果
