...
param(
    ...
    [string]$Category = "all",  # all, people, projects, tech, decisions
...
Write-Host "🔍 开始混合搜索: '$Query'" -ForegroundColor Cyan
Write-Host "📂 搜索范围: $Category" -ForegroundColor Gray

# ========== Layer 1: Memory System (语义搜索) ==========
Write-Host "`n[Layer 1] 搜索 Memory System..." -ForegroundColor Yellow

# 注意: memory_search 是 QClaw 工具，这里仅记录搜索逻辑
# 实际使用时需要在 QClaw 对话中调用 memory_search 工具
$searchCmd = "memory_search --query=`"$Query`" --maxResults=$MaxResults"
Write-Host "  命令: $searchCmd" -ForegroundColor Gray
Write-Host "  范围: MEMORY.md, memory/*.md" -ForegroundColor Gray

# ========== Layer 3: Custom KB (关键词搜索) ==========
Write-Host "`n[Layer 3] 搜索 Custom KB..." -ForegroundColor Yellow
...
switch ($Category) {
    ...
    "people" {
        $searchDirs = @("$workspaceRoot\kb\people")
    }
    "projects" {
        $searchDirs = @("$workspaceRoot\kb\projects")
    }
    "tech" {
        $searchDirs = @("$workspaceRoot\kb\tech")
    }
    "decisions" {
        $searchDirs = @("$workspaceRoot\kb\decisions")
    }
    default {
...
if ($results.Count -eq 0) {
    ...
    Write-Host "✅ 找到 $($results.Count) 个匹配结果`n" -ForegroundColor Green
