...
# 构造完整的 msgContent（参照 task_pusher.py 的 format_task_data）
import time as t
msg_id = f"msg_{int(t.time())}"
schedule_task_id = f"sched_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
task_finish_time = int(t.time())  # 秒级时间戳（必填！）
...
push_data = {
    ...
    "msgContent": [
        {
            "msgId": msg_id,                      # 必填
            "scheduleTaskId": schedule_task_id,    # 必填
            "scheduleTaskName": task_name,         # 必填！
            "summary": task_name[:100],           # 必填
            "result": task_data.get('task_result', '任务已完成'),  # 必填
            "content": task_data.get('task_content', ''),          # 必填
            "source": 'QClaw',                    # 必填
            "taskFinishTime": task_finish_time     # 必填！（整数秒级时间戳）
        }
    ]
}

# 按照 hiboards_client.py，包一层 data
...
print(f"  scheduleTaskId: {schedule_task_id}")
print(f"  taskFinishTime: {task_finish_time}")
