...
## 🧠 Harness Engineering 状态检查（每日触发）

**每次心跳时，检查Harness工程组件的运行状态：**

### L2安全反射层状态
- [ ] `SAFETY_REFLEX.md` 是否存在且≤15行
- [ ] 本次对话中是否有触发安全反射拦截的事件
- [ ] 安全反射日志是否已写入 `memory/safety-reflex-logs/`

### 情感状态检查（Phase 1）
- [ ] 读取 `memory/emotional-state.json` 状态记录
- [ ] 检查当前情感状态是否处于高风险标记(P<-0.5且A>0.5)
- [ ] 高风险状态→自动触发反思

### Plan-First 模式检查
- [ ] 是否由心跳触发的计划→执行分离任务
- [ ] 当前步骤是否一致（Plan→Execute→Verify）
