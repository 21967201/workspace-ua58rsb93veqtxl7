...
### ⚡ 规则5: 智能推理优先原则（2026-06-09 新增v2）

**MANDATORY: 所有推理任务必须遵循以下原则。**

#### 5.1 Token预算约束表

| 任务类型 | Token预算 | 推理策略 | 判断标准 |
|---------|-----------|---------|---------|
| **简单查询** | 0推理token | **直答** | 事实性查询,无需推理,已知知识 |
| **中等推理** | ≤7.6%原始CoT | **CoD草稿链** | 需要多步但不需深度推理 |
| **复杂推理** | ≤15%原始CoT | **精简推理+TokenSkip** | 跨Agent/多步骤/代码生成 |
| **关键决策** | ≤15%原始CoT | **多视角验证+精简推理** | 涉及资金/数据/权限/对外发布 |

- **策略优先级**: 直答(0 token) > CoD草稿链(7.6%) > 精简推理(15%) > 完整CoT
- **任何超过预算的推理**必须先记录理由
...
- 每步只写关键结论，**不展开冗余解释**
- 最终答案前用 **一句话验证逻辑自洽性**
- 若需要详细推理，先用精简版确认方向，再选择性展开关键步骤

#### 5.3 置信度标注
- **每个结论**标注: 🔴高 / 🟡中 / 🟢低
- **不确定场景**: 必须标注置信度 + 信息来源
- **事实性断言**: 必须有引用来源(论文/官方文档/GitHub)
- **关键决策**: 使用 `reasoning-personas` 切换至少2个视角验证

#### 5.4 自动反思
- **每次错误**写入 `self-improving/corrections.md`
- **每次失败任务**写入 `memory/failures/YYYY-MM-DD-任务名.md`
- **每周一**执行自动反思: 提取错误模式 → 更新memory.md

#### 5.5 进化监控与技术突破集成
- **每周一**执行技术突破搜索(openclaw-evolution-researcher)
- **评估标准**: 兼容性(0-10) + 收益(0-10) + 成本(0-10,反向)
- **P0级**(兼容≥7+收益≥7+成本≤3) → **24小时内自动集成**
- **P1级**(收益≥6) → **本周评估**
- **P2级** → **月度汇总**
- **集成后必须更新相关任务配置**
...
- ✅ 时间范围: 周一至周六（工作日+周六）
- ✅ 时间限制: 必须在10:30-18:10之前执行
- ❌ 禁止时间: 禁止在周日执行，禁止在18:10之后执行
- ❌ 禁止时间: 禁止在10:30之前执行（早晨太早）
...
## 📝 Automatic Capture Rules (2026-06-01 新增)

**MANDATORY: 在每次对话中，主动检测并捕获以下信息到 `memory/` 对应目录。**

### 捕获触发条件

#### 1. 人物信息捕获
**触发关键词**: 人名（中文/英文）、"同事"、"朋友"、"家人"、"团队成员"、"联系"、"电话"、"邮箱"、"微信"
...
```
用户: "张三今天帮我解决了服务器问题"
→ 立即捕获到 memory/people/2026-06-01-张三.md
```

---

#### 2. 项目信息捕获
**触发关键词**: "项目"、"任务"、"工作"、"需求"、"开始"、"完成"、"交付"、"上线"、"进度"、"里程碑"、"截止日期"
...
→ 立即捕获到 memory/projects/2026-06-01-GBrain全景管理.md
```

---

#### 3. 技术信息捕获
**触发关键词**: 技术名词（Python、React、AI、API等）、"学习"、"研究"、"尝试"、"使用"、"框架"、"工具"、"库"、"平台"
...
→ 立即捕获到 memory/tech/2026-06-01-GBrain.md
```

---

#### 4. 决策信息捕获
**触发关键词**: "决定"、"选择"、"方案"、"权衡"、"放弃"、"采用"、"优化"、"改进"、"原因"、"考虑"、"评估"
...
→ 立即捕获到 memory/decisions/2026-06-01-知识库方案选择.md
```
...
1. **立即捕获**: 检测到关键词后，立即写入文件（不要等对话结束）
2. **去重处理**: 同一天同一主题只捕获一次（追加更新，不重复创建）
3. **异步写入**: 捕获操作不阻塞对话，后台静默写入
4. **结构化格式**: 严格按照上述格式写入，便于后续搜索和整理
5. **每周整理**: 每周一自动执行 `kb/weekly_organize.py`，将重要信息从 `memory/` 迁移到 `kb/`
