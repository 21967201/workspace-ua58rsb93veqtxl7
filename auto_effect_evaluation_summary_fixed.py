...
def main():
        ...
        evaluation_metrics = [
            ...
            },
            {
                ...
                "metrics": [
                    ...
                    {"name": "任务完成率", "before": 85.0, "after": 95.0, "unit": "%", "improvement": 11.8}
                ]
            },
            {
                "category": "系统稳定性",
                "metrics": [
                    ...
                    {"name": "可用性", "before": 99.0, "after": 99.9, "unit": "%", "improvement": 0.9}
                ]
            }
        ]
        ...
        for category in evaluation_metrics:
            ...
            for metric in category["metrics"]:
                ...
                metric_result = {
                    ...
                    "is_target_met": actual_improvement >= metric["improvement"] * 0.9
                }
    ...
    try:
        ...
        for exp in experiences:
            # 模拟提炼过程
            refined_experiences.append({
                "aspect": exp["aspect"],
                "key_experience": exp["experience"],
                "key_lesson": exp["lesson"],
                "rating": exp["rating"],
                "is_replicable": exp["rating"] >= 4.0
            })
        ...
        # 统计可复用经验
        replicable_count = sum([1 for e in refined_experiences if e["is_replicable"]])
        total_exp_count = len(refined_experiences)
    ...
    try:
        ...
        for plan in next_phase_plans:
            ...
            print(f"{status} {plan['phase']} ({plan['focus']}): {len(plan['actions'])}个行动 (可行性: {feasibility_score:.2f})")
...
"""
        ...
        for result in evaluation_results:
            report_content += f"""#### {result['category']}
- **平均改善**: {result['average_improvement']:.2f}%
- **评估结论**: {result['evaluation']}
- **指标详情**:
"""
            for metric in result["metrics"]:
                report_content += f"  - {metric['name']}: {metric['before']}{metric['unit']} → {metric['after']}{metric['unit']} (改善: {metric['actual_improvement']:.2f}%)\n"
            report_content += "\n"
        
        report_content += f"""### 评估统计
...
"""
        ...
        report_content += f"""### 经验统计
- **经验总结数**: {len(refined_experiences)}
...
"""
        ...
        # 添加改进计划
        for i, plan in enumerate(next_phase_plans):
            report_content += f"""#### {plan['phase']} ({plan['focus']})
- **行动数**: {len(plan['actions'])}
- **预期收益**: {plan['expected_benefits']}
- **可行性**: {'可行' if plan_results[i]['is_feasible'] else '不可行'} (评分: {plan_results[i]['feasibility_score']:.2f})
- **具体行动**:
"""
