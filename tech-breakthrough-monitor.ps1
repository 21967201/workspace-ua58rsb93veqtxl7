...
try {
    ...
    if ($breakthroughs.Count -gt 0) {
        # 生成Markdown报告
        $reportContent = "# 技术突破监控报告 ($today)`n`n"
        $reportContent += "## 1. 技术突破列表`n`n"
        $reportContent += "| # | 技术名称 | 来源 | 发布时间 | 核心创新 | 优先级 |`n"
        $reportContent += "|---|---------|------|---------|---------|--------|`n"
        
        $i = 1
        foreach ($item in $breakthroughs) {
            $reportContent += "| $i | $($item.Title) | $($item.Source) | $($item.Published) | $($item.Details) | $($item.Priority) |`n"
            $i++
        }
        
        $reportContent += "`n## 2. 51指标评估`n`n"
        $reportContent += "| 技术名称 | 综合评分 | 优先级 | 评估详情 |`n"
        $reportContent += "|---------|------------|----------|----------|`n"
        
        foreach ($item in $breakthroughs) {
            $reportContent += "| $($item.Title) | $($item.Score) | $($item.Priority) | $($item.Details) |`n"
        }
        ...
        # 更新记忆文件
        $memoryContent = "`n## 技术突破监控 - $today`n`n$reportContent"
        ...
        # 更新记忆文件（即使没有突破也要记录）
        $memoryContent = "`n## 技术突破监控 - $today`n`n无新技术突破发现。`n"
        Add-Content -Path $memoryFile -Value $memoryContent
    }
